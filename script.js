...
// --- parseCSV function (FIXED Trailing Comma Discrepancy) ---
function parseCSV(csv) {
    ...
    const parseLine = (line, lineNumber) => {
        ...
        try {
            ...
            // *** REMOVED the explicit trailing comma handling here ***
            // We will handle discrepancies after parsing based on header count
        } catch (e) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        let values = parseLine(lines[i], currentLineNumber); // Use 'let' to allow modification
        ...
        // --- FIX: Handle discrepancy caused by potential trailing comma in data vs header ---
        if (valueCount === headerCount + 1 && values[valueCount - 1] === '') {
            console.log(`parseCSV: Line ${currentLineNumber} had ${valueCount} values (expected ${headerCount}), last was empty. Adjusting.`);
            values.pop(); // Remove the extra empty field from the end
            valueCount--; // Update the count
        }
        // --- END FIX ---

        // Now, perform the strict check with the potentially adjusted value count
        if (valueCount === headerCount) {
            const row = {};
            for (let j = 0; j < headerCount; j++) {
                const headerName = headers[j];
                // Basic check for valid header name (can be stricter if needed)
                if (headerName !== undefined && headerName !== null && headerName.trim() !== '') {
                    row[headerName] = values[j];
                } else {
                     console.warn(`parseCSV: Invalid header name at index ${j} while processing line ${currentLineNumber}.`);
                     // Decide how to handle: skip row, skip column, etc.
                }
            }
            data.push(row);
        } else {
            // Log detailed mismatch information if still not matching after adjustment
            console.warn(
                `parseCSV: Skipping line ${currentLineNumber} AFTER ADJUSTMENT CHECK. ` +
                `Expected ${headerCount} values, but found ${valueCount}. ` +
                `\n  Line content: "${lines[i]}"` +
                `\n  Parsed values: [${values.join(' | ')}]`
            );
        }
    }
