...
function handleDetailsClick(event) {
    ...
    const rowData = originalData.find(item => `${item['Model No.']}__${item['SoCset']}` === uniqueId);

    if (rowData) displayProductDetails(rowData);
    else console.error(`Data for ID "${uniqueId}" not found.`);
}
...
function displayProductDetails(rowData) {
    ...
    const socSet = rowData['SoCset'];
    const uniqueId = `${modelNo}__${socSet}`;

    const existingDetailBox = detailsContainer.querySelector(`.detail-box[data-unique-id="${uniqueId}"]`);
    ...
    detailBox.dataset.uniqueId = uniqueId;
...
function updateSelectedDetailHeader() {
    ...
    if (detailBoxes.length > 0) {
        detailBoxes.forEach(box => {
            const uniqueId = box.dataset.uniqueId;
            const rowData = originalData.find(item => `${item['Model No.']}__${item['SoCset']}` === uniqueId);

            if (rowData) {
                const modelNo = rowData['Model No.'];
                const socSet = rowData['SoCset'] || 'N/A';
                ...
                removeBtn.dataset.uniqueId = uniqueId; // Use unique ID here too
                removeBtn.onclick = (event) => {
                    const idToRemove = event.target.dataset.uniqueId;
                    const boxToRemove = detailsContainer.querySelector(`.detail-box[data-unique-id="${idToRemove}"]`);
                    if (boxToRemove) {
                        boxToRemove.remove();
                        updateSelectedDetailHeader();
                    }
