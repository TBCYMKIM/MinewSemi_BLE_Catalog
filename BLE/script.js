...
function getComparisonData() {
    ...
    products.sort((a, b) => {
        ...
        return String(a['Model No.']).localeCompare(String(b['Model No.']));
    });
...
function renderComparisonView() {
    ...
    const specOrder = ['Action', 'Image', ...csvHeaders, 'Inquiry'];

    const specsDiv = document.createElement('div');
    specsDiv.className = 'comparison-specs';
    specOrder.forEach(spec => {
        const specNameDiv = document.createElement('div');
        specNameDiv.textContent = spec === 'Action' || spec === 'Inquiry' ? '' : spec + ':';
        if (spec === 'Image') specNameDiv.classList.add('spec-image-header');
        if (spec === 'Action') specNameDiv.classList.add('spec-action-header');
        if (spec === 'Inquiry') specNameDiv.classList.add('spec-inquiry-header');
        specsDiv.appendChild(specNameDiv);
    });
    ...
    productsToCompare.forEach(product => {
        ...
        specOrder.forEach(spec => {
            ...
            productDiv.appendChild(valueDiv);
        });
