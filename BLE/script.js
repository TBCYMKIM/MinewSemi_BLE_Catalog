...
function renderDataTable(data) {
    ...
    data.forEach(item => {
        ...
        const btn = document.createElement('button'); btn.textContent = 'Add'; btn.className = 'details-button';
...
function renderComparisonView() {
    ...
    stickySpecHeaderDiv.classList.add('spec-sticky-header');
    stickySpecs.forEach(spec => {
        const specNameDiv = document.createElement('div');
        specNameDiv.textContent = spec === 'Action' ? '' : spec + ':';
         if (spec === 'Image') specNameDiv.style.height = '150px';
         else specNameDiv.style.height = '40px';
    ...
    inquirySpecHeaderDiv.classList.add('spec-inquiry-header');
